...
/**
 * Window Management class, provides a simpe warpper
 * around SDL_Window and SDL_Renderer functionalities
 */
class Window {
	public:
		/**
		...
		* @param dstRect The destination position and width/height to draw the texture with
		...
		/**
 		* Loads an image directly to tecture usning SDL_image's
 		...
 		*/	
		static SDL_Texture* RenderText(const std::string &message, const std::string &fontFile, SDL_Color, int fontSize);
		//Clear the renderer 
		static void Clear();
		//Present renderer, ie. update screen
		static void Present();
